...
with tab2:
    ...
    with main_col:
        # 1. 页面主标题和副标题
        st.markdown('<h1 class="vyron-title">三角洲干员「威龙」数字档案</h1>', unsafe_allow_html=True)
...
with tab4:
    ...
    # 初始化会话状态的索引
    if 'tab4_photo_ind' not in st.session_state:
        st.session_state['tab4_photo_ind'] = 0

    # 显示当前索引对应的图片和标题
    st.image(
        image_ua[st.session_state['tab4_photo_ind']]['url'],
        caption=image_ua[st.session_state['tab4_photo_ind']]['text']
    )

    # 定义"下一张"函数
    def next_tab4_photo():
        st.session_state['tab4_photo_ind'] = (st.session_state['tab4_photo_ind'] + 1) % len(image_ua)
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.button('上一张', use_container_width=True, on_click=prev_tab4_photo, key="tab4_prev_photo")
    with c2:
        st.button('下一张', use_container_width=True, on_click=next_tab4_photo, key="tab4_next_photo")
...
with tab5:
    ...
    # 初始化会话状态的索引
    if 'tab5_music_ind' not in st.session_state:
        st.session_state['tab5_music_ind'] = 0

    # 显示当前索引对应的图片和标题
    st.image(
        music_ua[st.session_state['tab5_music_ind']]['url'],
        caption=music_ua[st.session_state['tab5_music_ind']]['text']
    )
    st.audio(music_ua[st.session_state['tab5_music_ind']]['audio_file'])

    # 定义"下一首"函数
    def next_tab5_music():
        st.session_state['tab5_music_ind'] = (st.session_state['tab5_music_ind'] + 1) % len(music_ua)
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.button('上一首', use_container_width=True, on_click=prev_tab5_music, key="tab5_prev_music")
    with c2:
        st.button('下一首', use_container_width=True, on_click=next_tab5_music, key="tab5_next_music")
...
with tab6:
    ...
    # 初始化会话状态
    if 'tab6_video_ind' not in st.session_state:
        st.session_state['tab6_video_ind'] = 0

    # 显示当前视频
    current_video = video_arr[st.session_state['tab6_video_ind']]
    ...
    for i in range(len(video_arr)):
        with cols[i]:
            if st.button(f"第{i + 1}集", key=f"tab6_video_episode_{i}"):
                st.session_state['tab6_video_ind'] = i
                st.rerun()
    ...
    with control_cols[0]:
        if st.button("上一集", disabled=(st.session_state['tab6_video_ind'] == 0), key="tab6_prev_video"):
            st.session_state['tab6_video_ind'] -= 1
            st.rerun()

    with control_cols[1]:
        st.write(f"当前: 第{st.session_state['tab6_video_ind'] + 1}集")

    with control_cols[2]:
        if st.button("下一集", disabled=(st.session_state['tab6_video_ind'] == len(video_arr) - 1), key="tab6_next_video"):
            st.session_state['tab6_video_ind'] += 1
            st.rerun()
...
with tab7:
    ...
    st.markdown('<div class="tab7-header"><h1>📄 个人简历生成器</h1></div>', unsafe_allow_html=True)

    # 创建两列布局
    col1, col2 = st.columns([1, 1])

    with col1:
        st.markdown('<div class="tab7-input-section">', unsafe_allow_html=True)
        st.header("个人信息输入")
        
        # 照片上传
        st.subheader("📷 个人照片")
        uploaded_photo = st.file_uploader("上传个人照片", type=['png', 'jpg', 'jpeg'], key="tab7_resume_photo")
        
        # 基本信息
        st.subheader("👤 基本信息")
        name = st.text_input("姓名", placeholder="请输入您的姓名", key="tab7_resume_name")
        age = st.number_input("年龄", min_value=18, max_value=100, value=25, key="tab7_resume_age")
        gender = st.radio("性别", ["男", "女","其他"], horizontal=True, key="tab7_resume_gender")
        phone = st.text_input("联系电话", placeholder="请输入手机号码", key="tab7_resume_phone")
        email = st.text_input("邮箱地址", placeholder="请输入邮箱地址", key="tab7_resume_email")
        address = st.text_input("居住地址", placeholder="请输入居住地址", key="tab7_resume_address")
        
        # 求职意向
        st.subheader("🎯 求职意向")
        job_position = st.text_input("期望职位", placeholder="如：Python开发工程师", key="tab7_resume_job")
        salary_expectation = st.selectbox("期望薪资", 
                                        ["请选择", "3K-5K", "5K-8K", "8K-12K", "12K-15K", "15K-20K", "20K以上"], key="tab7_resume_salary")
        work_location = st.multiselect("期望工作地点", 
                                     ["北京", "上海", "广州", "深圳", "杭州", "南京", "成都", "武汉", "西安", "南宁"], key="tab7_resume_location")
        
        # 语言能力
        st.subheader("🌐 语言能力")
        
        # 初始化语言技能会话状态
        if 'tab7_resume_language_skills' not in st.session_state:
            st.session_state.tab7_resume_language_skills = []
        
        # 添加语言技能
        new_language = st.text_input("添加语言", placeholder="如：中文、英语、日语等", key="tab7_resume_language_input")
        
        if new_language:
            if st.button("➕ 添加语言", key="tab7_resume_add_language_btn"):
                if new_language not in st.session_state.tab7_resume_language_skills:
                    st.session_state.tab7_resume_language_skills.append(new_language)
                    st.rerun()
        
        # 显示语言标签
        if st.session_state.tab7_resume_language_skills:
            st.write("**已添加的语言：**")
            cols_per_row = 3
            for i in range(0, len(st.session_state.tab7_resume_language_skills), cols_per_row):
                cols = st.columns(cols_per_row)
                for j, language in enumerate(st.session_state.tab7_resume_language_skills[i:i+cols_per_row]):
                    with cols[j]:
                        if st.button(f"{language} ❌", key=f"tab7_resume_lang_{i+j}", 
                                   help=f"点击删除 {language}",
                                   use_container_width=True):
                            st.session_state.tab7_resume_language_skills.remove(language)
                            st.rerun()
        ...
        # 初始化会话状态
        if 'tab7_resume_computer_skills' not in st.session_state:
            st.session_state.tab7_resume_computer_skills = []
        ...
        # 过滤掉已选择的技能
        remaining_skills = [skill for skill in available_skills if skill not in st.session_state.tab7_resume_computer_skills]
        ...
        if remaining_skills:
            ...
            for i in range(0, len(remaining_skills), cols_per_row):
                ...
                for j, skill in enumerate(remaining_skills[i:i+cols_per_row]):
                    with cols[j]:
                        if st.button(f"➕ {skill}", key=f"tab7_resume_add_{skill}", 
                                   help=f"点击添加 {skill}",
                                   use_container_width=True):
                            st.session_state.tab7_resume_computer_skills.append(skill)
                            st.rerun()
        else:
            st.info("所有技能都已添加完毕！")
        
        # 显示已添加的技能标签
        if st.session_state.tab7_resume_computer_skills:
            st.write("**已添加的技能：**")
            
            # 使用columns来显示标签
            cols_per_row = 3
            skills = st.session_state.tab7_resume_computer_skills
            ...
            for i in range(0, len(skills), cols_per_row):
                ...
                for j, skill in enumerate(skills[i:i+cols_per_row]):
                    with cols[j]:
                        # 创建带删除按钮的技能标签
                        if st.button(f"{skill} ❌", key=f"tab7_resume_skill_{i+j}", 
                                   help=f"点击删除 {skill}",
                                   use_container_width=True):
                            st.session_state.tab7_resume_computer_skills.remove(skill)
                            st.rerun()
        ...
        personal_intro = st.text_area("个人简介", 
                                    ...
                                    height=100, key="tab7_resume_intro")
        
        # 工作经历
        st.subheader("💼 工作经历")
        work_experience = st.text_area("工作经历", 
                                     placeholder="请按时间倒序填写工作经历，包括公司名称、职位、工作时间和主要职责...",
                                     height=120, key="tab7_resume_work")
        
        # 教育背景
        st.subheader("🎓 教育背景")
        education = st.selectbox("最高学历", ["请选择", "高中", "大专", "本科", "硕士", "博士"], key="tab7_resume_education")
        school = st.text_input("毕业院校", placeholder="请输入毕业院校名称", key="tab7_resume_school")
        major = st.text_input("所学专业", placeholder="请输入专业名称", key="tab7_resume_major")
        graduation_date = st.date_input("毕业时间", value=date.today(), key="tab7_resume_graduation")
    ...
    with col2:
        ...
        # 语言能力
        if st.session_state.get('tab7_resume_language_skills'):
            st.subheader("🌐 语言能力")
            language_tags = ""
            for language in st.session_state.tab7_resume_language_skills:
                language_tags += f'<span class="tab7-skill-tag">{language}</span> '
            st.markdown(language_tags, unsafe_allow_html=True)
        
        # 计算机技能
        if st.session_state.get('tab7_resume_computer_skills'):
            st.subheader("💻 计算机技能")
            skill_tags = ""
            for skill in st.session_state.tab7_resume_computer_skills:
                skill_tags += f'<span class="tab7-skill-tag">{skill}</span> '
